...
- `valid/` — at least **13** verdicts that MUST validate, with expected signature verification results. The valid suite MUST include at least one verdict per composite-verdict label: `PROCEED`, `PROCEED_STRATEGIC`, and `ABANDON` (plus a `status: "partial"` example for the engine-uncertain case). The `status: "partial"` acceptance test (`valid/A4-partial-aiyana.json`) corresponds to Appendix §B.4 of this document and MUST be accepted by conformant consumers without coercion to a `complete` status or a different composite label.
